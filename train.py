...
def evaluate_policy(agent, env_name, model_type, max_action, step_count):
    ...
    eval_env = gym.wrappers.RecordVideo(eval_env, video_folder=video_folder, disable_logger=True)
...
def train(model_type, env_name, timesteps=None, wandb_project="RL_Assignment4"):
    ...
    # CRITICAL FIX: Safe max_action extraction to prevent NaNs
    max_action = float(env.action_space.high[0])
    if np.isnan(max_action) or np.isinf(max_action):
        max_action = 1.0
...
if __name__ == "__main__":
    ...
    parser.add_argument("--wandb_project", type=str, default="RL_Assignment4", help="WandB Project Name")
    
    args = parser.parse_args()
    
    train(args.model, args.env, timesteps=args.timesteps, wandb_project=args.wandb_project)
